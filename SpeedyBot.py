...
status = cycle(['para obter ajuda: %Ajuda'])


@client.event
async def on_ready():
    mudar_status.start()
    print('O Speedy está Pronto para entrar em ação')
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        await ctx.send('```Não podes executar este comando devido a falta de permissão neste servidor```')
    if isinstance(error, commands.BotMissingPermissions):
        await ctx.send('```o bot não pode executar este comando devido a falta de permissão neste servidor```')

#quando a mensagem cujo o id que esta no arquivo que ira ser criado ao iniciar o codigo sera adicionado um cargo com nome da reação, emoji.
@client.event
...
- %Creditos, Exibe um texto com o informações sobre mim e sobre meu criador.
 Comandos moderacão:
- %Clear quantidade, ira limpar as mensagens anteriores de acordo com o definido, quantidade padrão (5), necessita que o autor tenha de permissão de Gerenciar mensagens.
- %Kick @membro razão, expulsara o membro marcado pela razão escrita, necessita que o autor tenha de permissão de Expulsar membros.
- %Ban @membro razão, ira banir o membro marcado pela razão escrita, necessita que o autor tenha de permissão de Banir membros.
- %Unban nome#0000, ira desbanir a pessoa que for colocado o nome e o # dela, necessita que o autor tenha de permissão de Banir membros.
- %Renomear, ira renomear o nome do canal atual para o nome que você escrever, necessita que o autor tenha de permissão de Gerenciar canais.
- %Topico, ira por o topico que escrever, necessita que o autor tenha de permissão de Gerenciar canais.
- %Chatlento 0 a 21600, ira por o modo chat lento no canal com o tempod de escrita sendo o valor em segundos que você inserir, necessita que o autor tenha de permissão de Gerenciar canais.```""")
...
async def user_info(ctx, user: discord.Member):
    ...
    embed.add_field(name="**Status do utilizador**", value=f"{ustatus}", inline=True)
...
async def mutar(ctx, user: discord.Member, tempo: int, operator:str = 's', *, razão:str = 'mutado'):
    ...
    embed = discord.Embed(title="utilizador mutado", colour=discord.Colour(0xf8db63), description=f"{user} foi mutado por: {razão}")
    ...
    embed = discord.Embed(title="utilizador desmutado", colour=discord.Colour(0xf8db63), description=f"{user} foi desmutado por: o tempo acabou")
    embed.set_author(name=f"{client.user.name}", url="https://discordapp.com", icon_url=client.user.avatar_url)
    embed.add_field(name="servidor", value=f"{ctx.guild}.", inline=True)
...
async def mutar_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        ...
        if str(error) == 'user is a required argument that is missing.':
            await ctx.send('esqueceste-te de marcar o utilizador')
        elif str(error) == 'tempo is a required argument that is missing.':
            await ctx.send('esqueceste-te de colocar o tempo')
    elif isinstance(error, commands.ConversionError):
        if str(error) == 'Converting to "int" failed for parameter "tempo".':
            await ctx.send('falha ao converter o tempo para inteiro, usa numeros')
    else: 
        print(error)

#comando para limpar chat reservado apenas pra quem tem a permissão de gerenciar mensagens, limpar quantidade.
@client.command()
...
async def Renomear(ctx, *,nome:str):
    nome = nome.replace(' ', '-')
    await ctx.send(f'```diff\nlembrando que o bot necessita de permissão para executar este processo\n-tentando: trocando o nome do canal para: {nome}\n```\n**processando...**')
...
@client.command()
@commands.has_permissions(manage_channels=True)
async def Chatlento(ctx, tempo=0):
    tempo = int(tempo)
